...
def constructor_update_rule_sindy(sindy_models):
  def update_rule_sindy(q, h, choice, prev_choice, reward, spillover_update):
      ...
      if prev_choice == 1 and 'xH' in sindy_models:
        action_update = sindy_models['xH'].predict(np.array([q]), u=np.array([prev_choice]).reshape(1, -1))[-1] - q  # get only the difference between q and q_update as h is later added to q
