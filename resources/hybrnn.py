...
class BiRNN(hk.RNNCore):
  ...
  def _value_rnn(self, state, value, action, reward):
    ...
    # pre_act_val = value * action   # (batch_s, n_actions); output --> (q1, q2, ..., qn) * (a1, a2, ..., an) = q1*a1 + q2*a2 + ... + qn*an with a being one-hot encoded
    ...
    inputs = jnp.concatenate(
        [action, reward[:, jnp.newaxis]], axis=-1)#, action_i[:, jnp.newaxis]], axis=-1)
