...
class RLRNN(BaseRNN):
    ...
    def value_network(self, state, value, action, reward):
        ...
        self.append_timestep_sample('xQr_r', value, value + (reward>0.5)*action*reward_update)  # only rewarded actions are updated; in case of non-binary reward: make binary condition like (r > 0)*action*reward_update
        self.append_timestep_sample('xQr_p', value, value + (reward<=0.5)*action*reward_update)  # only penalized actions are updated; in case of non-binary reward: make binary condition like (r <= 0)*action*reward_update
