...
def load_dataset_from_csv(file_path, dataset_index=None):
    ...
    for session_id in session_ids:
        ...
        x_tensor = torch.tensor(x_trials, dtype=torch.float32).unsqueeze(0)  # (1, T, 9)
        y_tensor = torch.tensor(y_trials, dtype=torch.float32).unsqueeze(0)  # (1, T, 2)
...
def main():
    """
    Loads ALL dataset CSVs, combines them into one big dataset, splits last 20% of
    each session as validation, runs a SINGLE hyperparameter optimization with
    50 trials, then trains one final "global" model using the best hyperparameters.
    """
    import optuna
    ...
    logging.info(f"Found {len(all_data_files)} files to combine: {[os.path.basename(f) for f in all_data_files]}")
    ...
    for file_path in all_data_files:
        ds, n_participants = load_dataset_from_csv(file_path)
        if len(ds) > 0:
            all_datasets.append(ds)
        max_participants = max(max_participants, n_participants)

    if len(all_datasets) == 0:
        logging.warning("No data found at all. Exiting.")
        return

    combined_xs = torch.cat([ds.xs for ds in all_datasets], dim=0)
    combined_ys = torch.cat([ds.ys for ds in all_datasets], dim=0)
    full_dataset = DatasetRNN(combined_xs, combined_ys)
    logging.info(f"Combined dataset shape: {full_dataset.xs.shape}")

    # 2) Split the last 20% of trials in each session for validation
    train_dataset, val_dataset = split_dataset_last_20_percent(full_dataset, fraction=0.2)
    if train_dataset is None or val_dataset is None:
        logging.warning("Unable to split combined dataset. Exiting.")
        return
    ...
    def objective(trial):
        ...
        model_rnn, optimizer_rnn, final_train_loss = fit_model(
            ...
            convergence_threshold=CONV_THRESHOLD_FIXED,
            verbose=False,
            scheduler=True
        )
    ...
    logging.info(f"BEST GLOBAL PARAMS: {best_params}")
    logging.info(f"BEST GLOBAL VAL LOSS: {best_val_loss:.7f}")

    # 4) Train final global model using best hyperparams
    final_model = RLRNN(
        n_actions=n_actions,
        n_participants=max_participants,
        hidden_size=hidden_size,
    ...
    val_loss = compute_loss(final_model, val_dataset, n_steps=16, l1_weight_decay=best_params['l1_weight_decay'])
    n_params = sum(p.numel() for p in final_model.parameters())
    bic_score = compute_bic(val_loss, n_data_val, n_params)

    logging.info(f"Final global validation loss: {val_loss:.7f}")
    logging.info(f"Final global BIC: {bic_score:.7f}")

    output_dir = os.path.join(current_dir, "results_all_combined")
    os.makedirs(output_dir, exist_ok=True)
    model_filename = os.path.join(output_dir, "rnn_model_all_combined.pt")
    torch.save(final_model.state_dict(), model_filename)
    logging.info(f"Saved global final model to {model_filename}")
    ...
    summary_path = os.path.join(output_dir, "summary_all_combined.json")
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=2)
    logging.info(f"Global approach summary saved to {summary_path}")
