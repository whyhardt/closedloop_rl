...
def main(data, model, output_file, job_id):
    ...
    ll, bic, aic = df['LL'].sum(), df['BIC'].sum(), df['AIC'].sum()
    print(f'RNN: LL = {ll} --- BIC = {bic} --- AIC = {aic}')
    # if agent_sindy is not None:
    #     df = get_scores_array(experiment, [agent_sindy]*len(experiment), [n_parameters_sindy]*len(experiment))
    #     # df.to_csv(output_file.replace('.', '_sindy.'))
    #     ll, bic, aic = df['LL'].sum(), df['BIC'].sum(), df['AIC'].sum()
    #     print(f'SINDy: LL = {ll} --- BIC = {bic} --- AIC = {aic}')
